...
function getPct(votes: number, totalVotes: number) {
...
export default function Vote() {
  ...
  } = useCandidates(debouncedSearch ? { search: debouncedSearch } : undefined);

  const { data: leaderboard, loading: leaderboardLoading, error: leaderboardError } = useLeaderboard();

  const totalVotes = leaderboard.reduce((s, c) => s + c.vote_count, 0);
      ...
      <div className="p-5 md:p-8 max-w-7xl mx-auto space-y-8">
        ...
        {activeTab === "Leaderboard" && (
          <div className="space-y-6">
            {leaderboardLoading ? (
              <div className="text-center py-10 text-gray-400 animate-pulse">Memuat leaderboard...</div>
            ) : leaderboardError ? (
              <div className="text-center py-10 text-red-500">{leaderboardError}</div>
            ) : (
              <>
                {/* Podium */}
                {podium.length > 0 && (
                    ...
                    <div className="flex items-end justify-center gap-2 sm:gap-6">
                      {podium.map((candidate, i) => {
                        ...
                        return (
                          <button
                            key={candidate.id}
                            onClick={() => router.push(`/vote/${candidate.slug}`)}
                          ...
                          >
                            ...
                            >
                              {getPct(candidate.vote_count, totalVotes)}%
                            </span>
                            ...
                            <div
                              ...
                              style={{
                                backgroundColor: `${MEDAL_COLOR[i]}22`,
                                border: `1px solid ${MEDAL_COLOR[i]}44`,
                              }}
                            >
                ...
                <div className="space-y-2">
                  {leaderboard.slice(3).map((candidate, i) => {
                    const rank = candidate.rank || i + 4;
                    const pct = parseFloat(getPct(candidate.vote_count, totalVotes));
                    return (
                      <button
                        key={candidate.id}
                        onClick={() => router.push(`/vote/${candidate.slug}`)}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1.5">
                            <p className="font-bold text-sm text-gray-800 truncate text-left">{candidate.name}</p>
        ...
        {activeTab === "People Choice" && (
          <>
            {candidatesLoading ? (
              ...
              <div className="text-center py-10 text-red-500">{candidatesError}</div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 lg:gap-6">
                {candidates.map((candidate) => (
                  <div
                    key={candidate.id}
                    onClick={() => router.push(`/vote/${candidate.slug}`)}
                    className="relative aspect-3/4 rounded-2xl overflow-hidden group cursor-pointer shadow-md hover:shadow-xl transition-all hover:-translate-y-1 bg-gray-100"
                  >
                    <Image
                      src={candidate.image}
                      alt={candidate.name}
